...
Status: all six training runs and their evaluations report completion in the returned transcript for jobs 852634–852638. The study used 6,041,129 live transitions and 39,792 optimizer steps. Both arms failed the registered screening criteria: final clean completion averaged 25.83% (every1) and 27.00% (every12), versus 25.50% initially. The receipt is `runs/cluster-receipt-852634-852638/receipt.json`; raw cluster artifacts have not yet been independently downloaded. Do not resubmit this completed study. The original upload/submission procedure below is retained for provenance; use the archive/download section to retrieve results.
