...
def test_categorical_ppo_training_reload_and_decentralized_native_action(tmp_path,centralized):
    ...
    configuration={"algorithm":"mappo" if centralized else "ppo", "action_reference":"direct"}
    actor=ManeuverActor(restored,mapping(),configuration)
    assert actor.observation_space==restored.observation_space["actor"]
    assert actor.action_space==spaces.Box(-1.,1.,(2,),dtype=np.float64)
    assert actor.configuration==configuration and actor.configuration is not configuration
    with pytest.raises(ValueError,match="exactly once"):
        ManeuverActor(restored,mapping(),{**configuration,"action_reference":"goal_offset"})
    with pytest.raises(ValueError,match="critic"):
        ManeuverActor(restored,mapping(),{**configuration,"algorithm":"sac"})
    local=observations["actor"][0]
    action=actor(local)
    assert actor.action_space.contains(action)
