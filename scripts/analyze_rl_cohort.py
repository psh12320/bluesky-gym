...
def analyze(plan, trials, training_source, evaluation_source, classical_source=None):
    ...
    for trial in trials:
        ...
        for key, default in (("exploration", "gaussian"), ("sde_weight_std", None), ("sde_sample_freq", None), ("traffic_position_scale", 1.0), ("gae_lambda", .95)):
            if config.get(key, default) != plan["config"].get(key, default):
                raise ValueError("Unregistered learner setting: " + key)
