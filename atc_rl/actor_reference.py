...
def match_initial_actor(model, checkpoint, output_directory):
    ...
    if (config.get('pretraining') is not None or config.get('pretrained_model') or
            entries[0].get('pretraining') is not None or entries[0].get('supervised_optimizer_steps', 0)):
        raise ValueError('Supervised actors require explicit pretraining lineage; they are not untrained references')
