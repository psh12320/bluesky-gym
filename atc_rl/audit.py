...
def audit(directory):
    ...
    from atc_rl.pretrained import audit_pretraining
    pretraining=audit_pretraining(directory,config)
    ...
    from atc_rl.learning_rate import validate_learning_rate
    for restored in (initial,model):
        validate_learning_rate(restored,config)
    ...
    if isinstance(distribution,list):
        actor_output=torch.cat([part.logits for part in distribution],dim=-1)
        weights=torch.arange(1,actor_output.shape[-1]+1,dtype=actor_output.dtype)
        probe=(actor_output*weights).sum()
    else:
        probe=distribution.mean.sum()
    actor_gradient=torch.autograd.grad(probe,observation['critic'],allow_unused=True)[0]
    ...
    return {'run_directory':str(directory),'algorithm':config['algorithm'],**exploration,
        'pretraining':pretraining,'initial_policy_is_untrained':pretraining is None,
        'gamma':model.gamma,'gae_lambda':model.gae_lambda,'learning_rate':model.learning_rate,
