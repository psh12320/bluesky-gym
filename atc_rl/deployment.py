...
class DecentralizedActor:
    ...
    def __call__(self, observation):
        ...
        # Match WorldPool transport before static or traffic action projection.
        return np.asarray(self.model.predict(inputs, deterministic=True)[0], dtype=np.float32)
    ...
    def load(cls, model_path):
        ...
        if configuration["algorithm"] not in ("ppo", "mappo", "sac"):
            raise ValueError("Expected a shared PPO, MAPPO or fresh SAC checkpoint")
        ...
        critical = ("atc_rl/policy.py", "atc_rl/goal_offset.py", "atc_rl/feature_mask.py", "atc_rl/traffic_scaling.py", "atc_rl/sac_support.py", "atc_rl/maneuvers.py")
        ...
        if configuration["algorithm"] == "sac":
            from atc_rl.sac_support import load_sac_for_inference
            model = load_sac_for_inference(path, configuration)
            return LocalSACActor(model, configuration, matches[0])
        ...
        if configuration.get("exploration") == "categorical":
            from atc_rl.maneuvers import ManeuverActor, ManeuverMapping
            return ManeuverActor(model, ManeuverMapping.from_specification(configuration["maneuver_mapping"]), configuration, matches[0])
        return cls(model, configuration, matches[0])


class LocalSACActor:
    """A shared SAC actor whose input contains only one aircraft observation."""
    def __init__(self, model, configuration, record):
        self.model, self.configuration, self.record = model, configuration, record
        self.observation_space, self.action_space = model.observation_space, model.action_space
    ...
    def __call__(self, observation):
        ...
        if local.shape != self.observation_space.shape or not np.isfinite(local).all():
            raise ValueError("Supply one finite local aircraft observation")
        # Match WorldPool transport before static or traffic action projection.
        return np.asarray(self.model.predict(local, deterministic=True)[0], dtype=np.float32)
...
def make_environment(kind, actor, n_agents=10):
    ...
    try:
        ...
        if config.get("exploration") == "categorical":
            ...
            dictionary = world.observation_space(world.possible_agents[0]) if kind == "ma" else world.observation_space
            actual = ManeuverMapping.from_schema(local_observation_schema(dictionary))
            if actual != actor.mapping:
                raise ValueError("Native maneuver observation layout differs from the checkpoint")
...
def load_policy(kind, model_path=None):
    ...
    if kind not in ("ma", "sa") or model_path is None:
        raise ValueError("Select a track and a saved shared PPO/MAPPO/fresh-SAC checkpoint")
