...
def validate_configuration(source,canonical,protocol,live):
    ...
    require_matching_positions(source,canonical)
    require_matching_positions(protocol,canonical)
    ...
    if live>0:fields += [('progress_scale',0.0),('reward_scale',1.0),('learning_rate',3e-4)]
...
def main():
    ...
    if results[0]['live']!=0:raise ValueError('Include the initial policy at zero additional RL experience')
    ...
    record={'training_run':str(args.training_run.resolve()),'algorithm':config['algorithm'],'training_seed':config['seed'],
        'initial_policy_is_untrained':config.get('pretraining') is None,
        'supervised_initialization':config.get('pretraining'),
        ...
        'checkpoint_reuse':'Reuse requires exact full policy tensors, live experience, action/support settings and evaluation source. Initial references require exact starting tensors; positive-experience points must also match reward transformations and learning rate. Supervised experience, if present, is recorded separately.'}
    ...
    for ax,(metric,title,scale) in zip(axes.flat,panels):
        ...
        ax.set_title(title);ax.set_xlabel('Additional RL aircraft transitions (millions)' if config.get('pretraining') else 'Active aircraft transitions (millions)');ax.grid(alpha=.2)
