...
def validate_training(root,plan,row):
    ...
    if config.get('gae_lambda',.95)!=.95:raise ValueError('Existing matrix protocol requires GAE lambda 0.95')
    if config.get('traffic_position_scale',1.0)!=1.0:raise ValueError('Existing matrix protocol does not include traffic position scaling')
    if config.get('exploration','gaussian')!='gaussian':raise ValueError('Existing matrix protocol does not include gSDE')
...
def verify_evaluation(root,plan,directory,row,stage,record=None,checkpoint=None):
    ...
    if protocol.get('traffic_position_scale',1.0)!=1.0:raise ValueError('Existing matrix protocol does not include traffic position scaling')
    if protocol.get('exploration','gaussian')!='gaussian':raise ValueError('Existing matrix protocol does not include gSDE')
