...
def validate_reference(root,row,config,auditor=None):
    ...
    if actual.get('gae_lambda',.95)!=.95:raise ValueError('Existing matrix protocol requires GAE lambda 0.95')
    if actual.get('traffic_position_scale',1.0)!=1.0:raise ValueError('Existing matrix protocol does not include traffic position scaling')
    if actual.get('exploration','gaussian')!='gaussian':raise ValueError('Existing matrix protocol does not include gSDE')
