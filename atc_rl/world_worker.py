...
def local_observation_schema(dictionary):
    """Describe the actual flattened local fields and appended task clock."""
    from gymnasium import spaces
    if not isinstance(dictionary, spaces.Dict):
        raise ValueError("Expected the simulator's dictionary observation space")
    fields = []
    cursor = 0
    for name, space in dictionary.spaces.items():
        size = spaces.flatdim(space)
        fields.append({"name": name, "offset": cursor, "size": size})
        cursor += size
    if any(field["name"] == "time_remaining_fraction" for field in fields):
        raise ValueError("The task clock must be appended exactly once")
    fields.append({"name": "time_remaining_fraction", "offset": cursor, "size": 1})
    return fields
...
def worker(connection,config,directory):
    ...
    try:
        ...
        while True:
            ...
            elif command=='observation_schema':
                schema=local_observation_schema(world.observation_space(agents[0]))
                assert sum(field['size'] for field in schema)==dim+1
                connection.send(('ok',schema))
