...
class ManeuverMapping:
    ...
    def from_specification(cls, specification):
        mapping = cls(specification["actor_dim"], specification["cosine_index"], specification["sine_index"])
        if mapping.specification() != specification:
            raise ValueError("Unsupported or altered maneuver specification")
        return mapping
...
class ManeuverActor:
    ...
    def __init__(self,model,mapping,configuration,record=None):
