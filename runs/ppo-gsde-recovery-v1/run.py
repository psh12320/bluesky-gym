"""Recover the interrupted gSDE seed cohort without altering any prior artifact."""
...
def main():
    ...
    for seed in plan["pending_seeds"]:
        for arm in plan["arm_order"][str(seed)]:
            folder=PARENT/f"seed-{seed}"/arm;folder.mkdir(parents=True,exist_ok=False)
            values={**plan["recovery_config"],**plan["arms"][arm],"seed":seed,"run_dir":str(folder/"train")}
            ...
            if seed==52300 and arm=="every1":
                previous=Path(plan["original_directory"])/"seed-52300/every1/train/initial-model.zip"
                if policy_fingerprint(previous)!=policy_fingerprint(folder/"train/initial-model.zip"):
                    raise ValueError("Recovered run changed its original initial policy tensors")
        ...
        with (PARENT/f"initialization-check-{seed}.json").open("x",encoding="utf-8") as f:
    ...
    for name,digest in plan["protected_original_files"].items():
        if hashlib.sha256((ROOT/name).read_bytes()).hexdigest()!=digest:
            raise ValueError("Original experiment artifact changed during recovery: "+name)
    (PARENT/"complete.json").write_text(json.dumps({"completed_recovery_seeds":plan["pending_seeds"],"reused_completed_seed":52200,"finished_at_utc":datetime.now(timezone.utc).isoformat()}),encoding="utf-8")
