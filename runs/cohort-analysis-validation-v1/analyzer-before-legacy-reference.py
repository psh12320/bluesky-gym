...
def analyze(plan, trials, training_source, evaluation_source):
    ...
    for trial in trials:
        ...
        for stage, folder in paths.items():
            ...
            if (protocol["seed"] != plan["evaluation"]["seed"]
                    or protocol["episodes"] != worlds or protocol["source_sha256"] != expected_evaluation):
            ...
            if stage == "classical":
                ...
                for key, default in (("algorithm", None), ("guidance", False), ("filter", False),
