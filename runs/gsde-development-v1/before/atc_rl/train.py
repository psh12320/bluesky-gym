...
def main():
    ...
    if args.actor_reference is not None:
        ...
        if (reference_config.get('initial_action_std',.6065306597126334)!=args.initial_action_std or
    ...
    config.update(world_seeds=seeds,actor_information=('own aircraft observation with zeroed conflict-feature channels plus remaining-time fraction' if args.mask_conflict_features else 'own aircraft observation with current-state closest-approach features plus remaining-time fraction' if args.conflict_features else 'own standard aircraft observation plus remaining-time fraction'),
        ...
        gamma=.996508469331006,gae_lambda=.95,learning_rate=3e-4,clip_range=.2,entropy_coefficient=.01,
    ...
    try:
        environment=WorldPool(args.workers,directory/'workers',guidance=args.guidance,filter=args.filter,progress_scale=args.progress_scale,action_reference=args.action_reference,conflict_features=args.conflict_features,mask_conflict_features=args.mask_conflict_features,static_filter=args.static_filter)
        ...
        model=PPO(AircraftPolicy,environment,seed=args.seed,device=args.device,n_steps=args.rollout_steps,
            batch_size=args.batch_size,n_epochs=args.epochs,learning_rate=3e-4,gamma=config['gamma'],gae_lambda=.95,
            clip_range=.2,ent_coef=.01,vf_coef=.5,max_grad_norm=.5,target_kl=.03,
            rollout_buffer_class=ActiveRolloutBuffer,policy_kwargs={'centralized':args.algorithm=='mappo','log_std_init':math.log(args.initial_action_std)},verbose=0)
