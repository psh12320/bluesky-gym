"""Execute the registered PPO relative-position-scale pilot."""
...
def main():
    ...
    integration=ROOT/"runs/position-scale-development-v1/integration-complete.json"
    checked=read(integration)
    assert checked["gaussian_default_initial_and_trained_policy_tensors_unchanged"]
    assert checked["scaled_and_unscaled_initial_full_policy_tensors_identical"]
    assert checked["matched_metric_values"]=={"unchanged_initial_ma":180,"trained_vector_native":180,"unchanged_initial_sa":18}
    ...
    archive=ROOT/"runs/onpolicy-position-source-v1.zip"
    ...
    for seed in plan["seeds"]:
        ...
        paths=[PARENT/f"seed-{seed}"/arm/"train/initial-model.zip" for arm in ("original","scaled")]
        states=[]
        for path in paths:
        ...
        for state in states:
            assert torch.allclose(state["log_std"].exp(),torch.full_like(state["log_std"],.05))
        ...
        with (PARENT/"initialization-check.json").open("x",encoding="utf-8") as f:
            json.dump({"changed_tensors":differences,"identical_initial_tensors":True,"same_parameter_count":True,
                       "traffic_position_scales":[1,20],"initial_action_std":.05,
