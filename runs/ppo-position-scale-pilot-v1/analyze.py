"""Analyze both registered input-conditioning arms without selecting a winning checkpoint."""
...
def main():
    ...
    left,right=arms["original"],arms["scaled"]
    ignore={"run_dir","traffic_position_scale","traffic_position_normalization_m"}
    if {k:v for k,v in left["config"].items() if k not in ignore}!={k:v for k,v in right["config"].items() if k not in ignore}:
        raise ValueError("Input-conditioning comparison changed another setting")
    ...
    for metric in left["data"]["initial"][3]:
        ...
        effects[metric]=paired_effect(gain["scaled"],gain["original"],indices)
    ...
    result={"seed":seed,"both_registered_arms_included":True,"arms":arm_results,"scaled_minus_original_learning":effects,
    ...
    print(json.dumps({"results":str(destination),"scaled_minus_original_learning":effects}))
