"""Analyze both registered exploration arms without selecting a winning checkpoint."""
...
def main():
    ...
    left,right=arms["std005"],arms["std020"]
    ignore={"run_dir","initial_action_std"}
    if {k:v for k,v in left["config"].items() if k not in ignore}!={k:v for k,v in right["config"].items() if k not in ignore}:
        raise ValueError("Exploration comparison changed another setting")
    if left["state"].keys()!=right["state"].keys() or [k for k in left["state"] if not torch.equal(left["state"][k],right["state"][k])]!=["log_std"]:
        raise ValueError("Initialization differs beyond exploration")
    effects={};arm_results={}
    for metric in left["data"]["initial"][3]:
        ...
        effects[metric]=paired_effect(gain["std020"],gain["std005"],indices)
    ...
    result={"seed":seed,"both_registered_arms_included":True,"arms":arm_results,"wider_minus_narrower_learning":effects,
            "only_initial_log_std_differs":True,"initial_physical_behavior_identical":True,
    ...
    print(json.dumps({"results":str(destination),"wider_minus_narrower_learning":effects}))
