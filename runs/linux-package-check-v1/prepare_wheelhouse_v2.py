...
wheelhouse=out/'wheelhouse-v2';wheelhouse.mkdir(exist_ok=False)
temporary=out/'download-v2-tmp';temporary.mkdir(exist_ok=False)
command=[sys.executable,'-m','pip','download','--disable-pip-version-check','--no-cache-dir',
         ...
         '--dest',str(wheelhouse),'--find-links',str(out/'zmq-build'),'--constraint',str(out/'constraints-linux.txt'),
...
state_path=out/'wheelhouse-v2-state.json'
...
try:
    ...
    with (out/'wheelhouse-v2-download.log').open('x',encoding='utf-8') as log:
    ...
    for path in sorted(wheelhouse.glob('*.whl')):
        ...
        elif canonicalize_name(name)=='zmq':
            build=json.loads((out/'zmq-build/build-audit.json').read_text())
            expected=build['wheel_sha256'];source='Locally built from published zmq-0.0.0.tar.gz; see zmq-build/build-audit.json'
        else:
    ...
    manifest={'files':files,'compressed_bytes':sum(x['compressed_bytes'] for x in files.values()),
              ...
              'all_downloaded_wheels_match_published_sha256':True,'locally_built_wheels':{'zmq':json.loads((out/'zmq-build/build-audit.json').read_text())},'native_dependency_resolution_pending':True}
    (out/'wheelhouse-v2-manifest.json').write_text(json.dumps(manifest,indent=2),encoding='utf-8')
    state.update(status='wheelhouse_ready',completed_at_utc=datetime.now(timezone.utc).isoformat(),
                 wheels=len(files),manifest_sha256=hashlib.sha256((out/'wheelhouse-v2-manifest.json').read_bytes()).hexdigest(),
