...
def main():
    ...
    parser.add_argument("group",choices=["g0-f0"])
    args=parser.parse_args()
    protocol=read(PARENT/"protocol.json")
    group=next(g for g in protocol["support_groups"] if g["name"]==args.group)
    sys.path.insert(0,protocol['training_source'])
    from atc_rl.cluster import verify
    verify(Path(protocol['training_source']));verify(Path(protocol['evaluation_source']))
    if hashlib.sha256((ROOT/'runs/onpolicy-learning-source-v2.zip').read_bytes()).hexdigest()!=protocol['training_bundle_sha256']:
        raise ValueError('Frozen training source archive changed')
    prior=protocol['prior_three_cell_results']
    ...
    for seed in protocol["seeds"]:
        ...
        state=initial_state(folder/'train/initial-model.zip')
        for row in protocol['rows']:
            if row['seed']==seed and not row['new_training']:
                reference=initial_state(Path(row['training'])/'initial-model.zip')
                if state.keys()!=reference.keys() or any(not torch.equal(state[key],reference[key]) for key in state):
                    raise ValueError('Full initialization differs from the existing support cells')
