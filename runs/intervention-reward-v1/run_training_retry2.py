...
ROOT=Path(__file__).resolve().parents[2];BASE=Path(__file__).resolve().parent;RUN=BASE/'training-retry2'
...
def training(stage):
    ...
    def capture_archive(directory):
        import platform
        directory=Path(directory)
        sources={name.removeprefix('source/'):candidate/name for name in manifest['files'] if name.startswith('source/')}
        sources.update({'_experiment/intervention_reward.py':BASE/'intervention_reward.py',
                        '_experiment/runner.py':Path(__file__), '_experiment/protocol.json':BASE/'protocol.json'})
        hashes={}
        with zipfile.ZipFile(directory/'source.zip','x',zipfile.ZIP_DEFLATED) as archive:
            for name,path in sorted(sources.items()):
                data=path.read_bytes();hashes[name]=hashlib.sha256(data).hexdigest();archive.writestr(name,data)
        metadata={'execution_source':'Immutable candidate archive plus the recorded experimental extension and driver',
            'candidate_archive_sha256':sha(ARCHIVE),'source_sha256':hashes,
            'python':platform.python_version(),'platform':platform.platform()}
        dump(directory/'provenance.json',metadata)
        packages=subprocess.check_output([sys.executable,'-m','pip','freeze'],text=True,encoding='utf-8')
        (directory/'packages.txt').write_text(packages,encoding='utf-8')
        return metadata
    trainer.capture=capture_archive
...
def parent():
    ...
    original_run=BASE/'execution'
    assert read(original_run/'state.json')['status']=='failed'
    original_plan=read(original_run/'execution-plan.json')
    assert original_plan['protocol_sha256']==sha(BASE/'protocol.json')
    assert original_plan['extension_sha256']==sha(BASE/'intervention_reward.py')
    assert original_plan['runner_sha256']==sha(BASE/'run_experiment.py')
    assert read(original_run/'validation-audit.json')['status']=='passed'
    assert not (original_run/'train_0/training/initial-model.zip').exists()
    assert not (original_run/'train_0/training/training_summary.json').exists()
    for label in ('validate_0','validate_1'):
        candidate=original_run/label/'candidate'
        verify_files(candidate,read(candidate/'manifest.json'))
    RUN.mkdir(exist_ok=False)
    plan={'created_at_utc':datetime.now(timezone.utc).isoformat(),'protocol_sha256':sha(BASE/'protocol.json'),
          'runner_sha256':sha(Path(__file__)),'extension_sha256':sha(BASE/'intervention_reward.py'),'stage_order':PROTOCOL['stage_order'][2:],
          'original_attempt_state_sha256':sha(original_run/'state.json'),
          'reused_validation_audit_sha256':sha(original_run/'validation-audit.json'),
          'recovery_scope':'Training had not started. Preserve validated physics/reward separation; replace Git-based capture with archive-based provenance. No policy, reward, budget or evaluation changes.'}
