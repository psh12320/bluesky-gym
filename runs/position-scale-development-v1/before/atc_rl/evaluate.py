"""Evaluate a PPO/MAPPO checkpoint or fixed benchmark on one seeded world stream."""
...
def main():
    ...
    environment=WorldPool(1,directory/'workers',guidance=config['guidance'],filter=config['filter'],
                          action_reference=config.get('action_reference','direct'),conflict_features=config.get('conflict_features',False),mask_conflict_features=config.get('mask_conflict_features',False),static_filter=config.get('static_filter',False))
    try:
        if model and model.observation_space!=environment.observation_space:
