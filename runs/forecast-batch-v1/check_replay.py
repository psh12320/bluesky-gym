...
import argparse,contextlib,hashlib,json,os,runpy,sys,time,zipfile
...
out=base/f'replay-{args.env}';out.mkdir(exist_ok=False);candidate=out/'candidate';candidate.mkdir()
...
protocol={'started_at_utc':datetime.now(timezone.utc).isoformat(),'track':args.env,'seed':2026,'episodes':2,
          'candidate_archive_sha256':sha(archive),'kernel_summary_sha256':sha(base/'summary.json'),
