...
def parent():
    ...
    dump(plan,{'created_at_utc':datetime.now(timezone.utc).isoformat(),'protocol_sha256':sha(BASE/'protocol.json'),
        ...
        'previous_failed_attempt_state_sha256':sha(BASE/'state.json'),'retry_reason':'Inspect the actual performance implementation through BlueSky getproxied; the earlier check inspected its Proxy type before any scenario ran','geometry_tests_passed':9,'arms':PROTOCOL['initial_arms'],'arm_order_fixed_before_results':True})
