...
for track,folder,interval in [('sa','heldout-2027-sa-reach250-v1',10),('ma','heldout-2027-ma-interval5-v1',5)]:
    ...
    try:
        ...
        results[track]={'model_sha256':freeze['model_sha256'],'geography_backend':geo.qdrdist.__module__,'simulator_elapsed_seconds':elapsed,'actual_simulator_step_seconds':float(bs.sim.simdt),'expected_decision_interval_seconds':interval,'selected_resolution_implementation':implementation.__module__+'.'+implementation.__qualname__,'bluesky_conflict_resolution_off':True}
    finally:env.close()
...
(Path(__file__).resolve().parent/'simulation-audit-canonical.json').write_text(json.dumps(out,indent=2),encoding='utf-8')
