...
clear overall advantage over that reference. Five-second control is a separate
ongoing ablation; use the tested ten-second recipe for the initial cluster trial.
...
Held-out seed 2027 and official seed 42 remain unused until candidate selection.
...
- `public_route_choice_fast_residual_reach250` changes only the arrival reward to
  ...
  SA transfer retains every arrival on twenty cases with lower intrusion; its
  broader comparison is pending. This is an ablation, not the default cluster
  configuration. All 89 regression tests pass after the recipe addition.
