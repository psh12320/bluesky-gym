"""Observe heading command strings in one frozen MA test scenario, without editing commands."""
...
try:
    ...
    env.close()
result={'checked_at_utc':datetime.now(timezone.utc).isoformat(),'scope':'Read-only command observation on seed-2027 scenario index 46, with prefix resets but prefix rollouts skipped; metric parity with the original full run is not assumed','model_sha256':freeze['model_sha256'],'geography_backend':geo.qdrdist.__module__,'controller_commands_modified':False,'heading_commands_checked':heading_count,'invalid_commands':invalid,'nonfinite_observation_count':nonfinite_observations,'nonfinite_action_count':nonfinite_actions,'final_records':final,'primary_full_run_metric_comparison':'pending completed CSV; skipped-prefix parity requires verification'}
