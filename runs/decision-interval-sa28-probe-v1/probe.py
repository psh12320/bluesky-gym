"""Compare control rate and learned adjustment on the exact failed SA scenario."""
...
report = dict(model_sha256=hashlib.sha256(model_path.read_bytes()).hexdigest(),
              ...
              scope='Selected SA failure, no population or additional-training claim',
              zero_control='Exactly zero residual output reproduces the fast classical route follower',
              arms={})
for name, interval, zero in [('learned10', 10, False), ('learned5', 5, False), ('zero10', 10, True)]:
    env = make_env('sa', 'public_route_choice_fast_residual_sa_transfer', guard_traffic=True)
    trace = []
    ...
    try:
        ...
        if name in expected:
            differences = {k: abs(record[k] - expected[name][k]) for k in METRICS}
            assert not any(differences.values()), differences
            result.update(all_nine_reference_metrics_exact=True, maximum_absolute_metric_differences=differences)
