...
PLAN = read(OUT / "integration-recovery-protocol.json")
...
def native():
    ...
    (directory / "simulator").mkdir(exist_ok=False)
...
def review():
    ...
    datasets = {name:rows(path) for name,path in {
        ...
        "vector":OUT / "vector/aircraft.csv", "native":OUT / "native-recovery/aircraft.csv"}.items()}
    ...
    result = {"status":"complete", "finished_at_utc":datetime.now(timezone.utc).isoformat(),
              "protocol_sha256":sha(OUT / "integration-recovery-protocol.json"),
              ...
              "vector":read(OUT / "vector/summary.json"), "native":read(OUT / "native-recovery/summary.json"),
              "setup_recovery":PLAN["setup_recovery"],
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        return
    (OUT / "native-recovery-started").mkdir(exist_ok=False)
    script_hash = sha(__file__)
    protocol_hash = sha(OUT / "integration-recovery-protocol.json")
    for stage in ("native", "review"):
        assert sha(__file__) == script_hash and sha(OUT / "integration-recovery-protocol.json") == protocol_hash
        with (OUT / f"{stage}-recovery.log").open("x", encoding="utf-8") as stream:
