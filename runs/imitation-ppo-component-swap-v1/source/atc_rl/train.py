...
def main():
    ...
    if args.pretrained_model is not None and (args.algorithm!='ppo' or args.actor_reference is not None or args.neutral_action_mean):
        parser.error('Pretraining requires PPO without another actor initialization')
    import math
    ...
    config.update(world_seeds=seeds,actor_information=('own aircraft observation with zeroed conflict-feature channels plus remaining-time fraction' if args.mask_conflict_features else 'own aircraft observation with current-state closest-approach features plus remaining-time fraction' if args.conflict_features else 'own standard aircraft observation plus remaining-time fraction'),
        ...
        gamma=.996508469331006,gae_lambda=args.gae_lambda,learning_rate=3e-4,clip_range=.2,entropy_coefficient=.01,
    ...
    try:
        ...
        model=PPO(AircraftPolicy,environment,seed=args.seed,device=args.device,n_steps=args.rollout_steps,
            batch_size=args.batch_size,n_epochs=args.epochs,learning_rate=3e-4,gamma=config['gamma'],gae_lambda=args.gae_lambda,
        ...
        if args.pretrained_model is not None:
            ...
            config['training_method']='ppo_after_behavior_cloning'
            (directory/'config.json').write_text(json.dumps(config,indent=2),encoding='utf-8')
